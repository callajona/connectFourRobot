...
int main(int argc, char** argv) {
  ...
  std::ofstream MyFile;
  MyFile.open("/home/project/projects/Solver_Pascal/LUT/LUT_2.txt",std::ios::app); // std::ios::app - non destrictive writing [app = append mode]
  ...
  for (int i_1 = 0; i_1 < 7; i_1++) {
    for (int i_2 = 0; i_2 < 7; i_2++) {
      ...
      std::string move = colNumbers[i_1] + colNumbers[i_2]; // Create the move
      int int_move = ((i_1 + 1)*1000) + ((i_2 + 1)*100);
      ...
      if (int_move > 51) {
        ...
        matching_move = readLUT_compareBitmap(bitmap_srt, 2);// Compare normal bitmap - find a matching move
        matching_Flipped_move = readLUT_compareBitmap(flipped_bitmap_srt, 2); // Compare flipped bitmap - find a flipped matching move
