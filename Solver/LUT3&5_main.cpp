...
int main(int argc, char** argv) {
	...
	std::cout << "It should finish computing at some point on the 1st of August - I will be back then to collect the results" << std::endl;
  ...
  std::ofstream MyFile;
  MyFile.open("/home/dvrkteam/Connect4Robot_Project/LUT/LUT_5.txt",std::ios::app); // std::ios::app - non destrictive writing [app = append mode]
  ...
  MyFile.close(); // Close the file 
	...
	std::cout << "\nLUT 5 Finished Computing" << std::endl;

  // ---------------------------------------------------LUT 3---------------------------------------------------

	std::cout << "\nStarting to Compute LUT 3" << std::endl;

  // Create and open a text file
  std::ofstream MyFile2;
  MyFile2.open("/home/dvrkteam/Connect4Robot_Project/LUT/LUT_3.txt",std::ios::app); // std::ios::app - non destrictive writing [app = append mode]
  ...
  for (int i_1 = 0; i_1 < 7; i_1++) {
    for (int i_2 = 0; i_2 < 7; i_2++) {
      for (int i_3 = 0; i_3 < 7; i_3++) {
        ...
        std::string move = colNumbers[i_1] + colNumbers[i_2] + colNumbers[i_3]; // Create the move
        int int_move = ((i_1 + 1)*100) + ((i_2 + 1)*10) + ((i_3 + 1)*1);
        ...
        if (int_move > 111) {
          ...
          // Compare the bitmaps
          std::string matching_move, matching_Flipped_move;
          matching_move = readLUT_compareBitmap(bitmap_srt, 3);// Compare normal bitmap - find a matching move
          matching_Flipped_move = readLUT_compareBitmap(flipped_bitmap_srt,3); // Compare flipped bitmap - find a flipped matching move
          ...
          // Write Score to file 
          MyFile2 << move << " " << bitmap_srt << " [";
          for (int i = 0; i < 6; i++) {
            MyFile2 << colScores[i] << " ";
          }
          MyFile2 << colScores[6] << "]" << std::endl; 
        }
      }
    }
  }
...
std::vector<int> readLUT(std::string move_to_find) {
  ...
  std::string filename = "/home/dvrkteam/Connect4Robot_Project/LUT/LUT_" + LUT_str + ".txt"; // Create file name to index correct LUT
